...
public class Test {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			input = Utils.coloredStringInput(scanner); // .trim(); to ignore the space
			...
			case "exit":
				...
				break;
			default:
				Utils.printError("不明なコマンドです。利用可能なコマンドを確認するには 'help' と入力してください。");
			}
		}
	}
	...
	private static void handleUserCommands(Scanner scanner) {
		...
		while (true) {
			...
			input = Utils.coloredStringInput(scanner);
		...
		while (true) {
			...
			if (input.equalsIgnoreCase("exit")) {
				...
				try {
					...
					Utils.printError("無効なコマンド形式です。次の形式を使用してください: sell item <item_id> to <buyer_id>");
				}
			} else {
				Utils.printError("不明なコマンドです。利用可能なコマンドを確認するには 'help' と入力してください。");
			}
